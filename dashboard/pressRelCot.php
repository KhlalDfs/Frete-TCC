<?php 
    ...
    $dados = "";
        ...
        $sqlSelect = "SELECT * from cotacao where idUser = $id";
        $result = mysqli_query($conn, $sqlSelect);
        while($row = mysqli_fetch_assoc($result)){
        $dados .= "<h4>Número da Cotação:  ". $row['idCot']."</h4>";
...
require_once 'dompdf/autoload.inc.php';
    ...
    $dompdf = new Dompdf();
    $dompdf->loadHtml(
        '<h1 style="text-align: center;">Relatório de Cotações Realizadas</h1><br>' . 
        $dados
    );
    $dompdf->render();
    $dompdf->stream(
        "Relatório_Cotacoes.pdf",
        array(
            "Attachment"=>true
        )
    );
