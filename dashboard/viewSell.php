<?php
   ...
   if($resultEdit->num_rows > 0){
    ...
    if($tipo == "A"){
        ...
        $nump = ceil($total/$qtd);
        $inicio = ($qtd*$pgn)-$qtd;
        $rest = "SELECT * from venda limit $inicio, $qtd";
        $result = mysqli_query($conn, $rest);
        $ver = mysqli_num_rows($result);
    }
        else{
        $result = "SELECT * from venda where idUser = $id";
                            ...
                            <table class="table" style="width: 90%;">  
                                <thead>
                                    <tr style="font-size: 8px;">
                                        ...
                                        <th>Atualizar Status da Venda</th>
                                        <th>Deletar</th>
                                    </tr>
                                </thead>
                                <tbody style="font-size: 13px;">
                                    ...
                                    <?php while($rows_users = mysqli_fetch_assoc($view)){?>
                                            <td><?php echo $rows_users['idSell'];?></td>
                                            <td><?php echo $rows_users['nome'];?></td>
                                            <td><?php 
                                                if ($rows_users['stats'] == 'P') {
                                                    $rows_users['stats'] = 'Pendente';
                                                }else if($rows_users['stats'] == 'C') {
                                                    $rows_users['stats'] = 'Concluido';
                                                }else  if($rows_users['stats'] == 'E') {
                                                    $rows_users['stats'] = 'Cancelado';
                                                }else {
                                                    echo 'Atualizar';
                                                }
                                                echo $rows_users['stats'];
                                            ?></td>
                                            <td><?php echo "R$ ".$rows_users['valor'];?></td>
                                            <td><?php 
                                            ...
                                            <?php  echo "<td><a href='stats.php?id=$rows_users[idSell]'>"?><i class="fa-solid fa-pen-to-square"></i></a></td>
                                            <?php  echo "<td><a href='deleteSell.php?id=$rows_users[idSell]' style='color: red'>"?><i class="fa-solid fa-trash"></i></a></td>
                                        </tr>
                                    <?php } ?>
                                </tbody>
                            </table>
